...
function getRandomCoords(rangeStart = 0, rangeEnd = 25) {
    return {
...
let game = {
    ...
    logic() {
        ...
        for (let i = 0; i < this.fruits.length; i++) {
            let fruitCoords = this.fruits[i];
            if (fruitCoords.x === snake.head.x && fruitCoords.y === snake.head.y) {
                this.score++;

                snake.tail.push({});
                this.fruits.splice(i, 1);
                this.fruits.push(getRandomCoords());
                break;
            }
        }
    },
